Attribute VB_Name = "WebDriverManager4SelenWrapper"
Option Explicit
...
Public Property Get WebDriverPath(Browser As BrowserName) As String
    ...
    Select Case Browser
        Case BrowserName.BN_Chrome: WebDriverPath = MyDocuments & "\WebDriver\chromedriver.exe"
        Case BrowserName.BN_Edge:   WebDriverPath = MyDocuments & "\WebDriver\edgedriver.exe"
    End Select
End Property
...
Public Sub InstallWebDriver(Browser As BrowserName, Optional DriverPathTo As String)
    ...
    Select Case Browser
        Case BrowserName.BN_Chrome: DriverVer = RequestWebDriverVersion(BrowserVer)
        Case BrowserName.BN_Edge:   DriverVer = BrowserVer
    End Select
...
Public Sub SafeOpen(Driver As WebDriver, Browser As BrowserName, Optional CustomDriverPath As String, Optional WebDriverOptions As WebDriverOptions = Nothing)
    ...
    Driver.OpenBrowser WebDriverOptions
...
Function IsLatestDriver(Browser As BrowserName, driverPath As String) As Boolean
    Select Case Browser
    Case BrowserName.BN_Edge
        IsLatestDriver = BrowserVersion(BN_Edge) = DriverVersion(driverPath)
    
    '// Chromeは末尾のバージョンがブラウザとドライバーで異なることがある
    Case BrowserName.BN_Chrome
        IsLatestDriver = RequestWebDriverVersion(BrowserVersion(BN_Chrome)) = DriverVersion(driverPath)
